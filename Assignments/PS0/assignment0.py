...
def animal_with_highest_average_hp(animals):
    ...
    chinchilla
    """
    max_hp = max(hp_average.keys())

    for average, species in hp_average.items():
        if average == max_hp:
...
def highest_pp_of_each_species(animals):
    ...
    c1
    """
    for species, pp2names in all_pp.items():
        max_pp = max(pp2names.keys())
        for pp, names in pp2names.items():
            if pp == max_pp:
